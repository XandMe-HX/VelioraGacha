...
public final class VelioraGachaPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void loadRewards() {
        ...
        for (String crate : root.getKeys(false)) { List<Reward> list = new ArrayList<>(); ConfigurationSection c = root.getConfigurationSection(crate); if (c == null) continue;
            for (String rarity : c.getKeys(false)) for (Map<?, ?> m : rewardsYml.getMapList("rewards." + crate + "." + rarity)) {
                ...
                Material icon = mat(str(m, "material", type.equals("COMMAND") ? "COMMAND_BLOCK" : "STONE"), type.equals("COMMAND") ? Material.COMMAND_BLOCK : Material.STONE);
                if (type.equals("COMMAND")) list.add(new Reward(RewardType.COMMAND, str(m, "display", "Command Reward"), icon, amount, weight, commands(m)));
                else list.add(new Reward(RewardType.ITEM, str(m, "display", icon.name() + " x" + amount), icon, amount, weight, List.of()));
            }
    ...
    private enum RewardType{ITEM,COMMAND} private record KeyDef(String id,String name,Material icon,double price){} private record CrateDef(String id,String name,Material block,String key,String effect,String level){} private record CrateLoc(String crate,String world,int x,int y,int z){} private record Reward(RewardType type,String display,Material icon,int amount,int weight,List<String> commands){ItemStack icon(){ItemStack it=new ItemStack(icon,Math.max(1,Math.min(64,amount)));ItemMeta im=it.getItemMeta();if(im!=null){im.setDisplayName(ChatColor.WHITE+ChatColor.stripColor(display));it.setItemMeta(im);}return it;}} private static final class Holder implements InventoryHolder{final String type,id;final int amount;Inventory inv;Holder(String t,String i,int a){type=t;id=i;amount=a;}public Inventory getInventory(){return inv;}}
}
