...
public final class VelioraGachaPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    private static final List<String> ORDER = List.of("shinjuore", "tenshiyoro", "kinzai", "kagekari", "kouzan");
    ...
    private void loadKeys() {
        ...
        for (String id : root.getKeys(false)) {
            String p = "keys." + id;
            tmp.put(normal(id), new KeyDef(normal(id), keysYml.getString(p + ".display-name", id), mat(keysYml.getString(p + ".material"), Material.TRIPWIRE_HOOK), keysYml.getDouble(p + ".price", 150000), keysYml.getBoolean(p + ".glow", true), keysYml.getStringList(p + ".lore")));
        }
        for (String crate : ORDER) {
    ...
    private void loadCrates() {
        ...
        if (root != null) {
            for (String id : root.getKeys(false)) {
                ...
                tmp.put(normal(id), new CrateDef(normal(id), cratesYml.getString(p + ".display", id), mat(cratesYml.getString(p + ".block"), Material.CHEST), normal(cratesYml.getString(p + ".key", id + "_key")), cratesYml.getString(p + ".effect", "fire_spiral"), cratesYml.getString(p + ".level-reward", "Default"), cratesYml.getBoolean(p + ".hologram-enabled", true)));
            }
        }
        for (String id : ORDER) if (tmp.containsKey(id)) crates.put(id, tmp.remove(id));
        crates.putAll(tmp);
        ConfigurationSection locs = cratesYml.getConfigurationSection("locations");
        if (locs != null) {
            for (String key : locs.getKeys(false)) locations.put(key, new CrateLoc(normal(cratesYml.getString("locations." + key + ".crate", "")), cratesYml.getString("locations." + key + ".world", "world"), cratesYml.getInt("locations." + key + ".x"), cratesYml.getInt("locations." + key + ".y"), cratesYml.getInt("locations." + key + ".z")));
        }
    }
    ...
    private void loadRewards() {
        ...
        if (root != null) {
            for (String crate : root.getKeys(false)) {
                List<Reward> list = rewards.computeIfAbsent(normal(crate), ignored -> new ArrayList<>());
                ConfigurationSection c = root.getConfigurationSection(crate);
                if (c == null) continue;
                for (String rarity : c.getKeys(false)) {
                    for (Map<?, ?> map : rewardsYml.getMapList("rewards." + crate + "." + rarity)) {
        ...
        if (guiRoot != null) {
            for (String crate : guiRoot.getKeys(false)) {
                for (String rarity : RARITIES) {
                    List<?> items = rewardsGuiYml.getList("rewards." + crate + "." + rarity + ".items", List.of());
                    for (Object object : items) {
                        if (object instanceof ItemStack item && item.getType() != Material.AIR) {
                            ItemStack copy = item.clone();
                            int weight = Math.max(1, rewardsGuiYml.getInt("editor.default-weight", 10));
                            rewards.computeIfAbsent(normal(crate), ignored -> new ArrayList<>()).add(new Reward(RewardType.ITEM, displayName(copy), copy.getType(), Math.max(1, copy.getAmount()), weight, List.of(), copy));
                        }
                    }
                }
            }
    ...
    private Reward manualReward(String crate, String rarity, Map<?, ?> map) {
        try {
            ...
            int amount = Math.max(1, num(map, "amount", 1));
            int weight = Math.max(1, num(map, "weight", 10));
            Material material = mat(str(map, "material", type.equals("COMMAND") ? "COMMAND_BLOCK" : "STONE"), type.equals("COMMAND") ? Material.COMMAND_BLOCK : Material.STONE);
            String display = str(map, "display", material.name() + " x" + amount);
            if (type.equals("COMMAND")) return new Reward(RewardType.COMMAND, display, material, amount, weight, commands(map), item(material, display, List.of("&7Command reward", "&7Rarity: &f" + rarity)));
            ...
            return new Reward(RewardType.ITEM, display, material, amount, weight, List.of(), item);
        } catch (Throwable throwable) {
    ...
    private boolean giveKey(CommandSender sender, String[] args) {
        ...
        if (args[1].equals("*")) {
            for (Player player : Bukkit.getOnlinePlayers()) giveOrDrop(player, createKeyItem(def, amount));
        ...
        giveOrDrop(target, createKeyItem(def, amount));
    ...
    private void openKeys(Player player) {
        ...
        int i = 0;
        for (CrateDef crate : orderedCrates()) {
    ...
    private void openPlacer(Player player) {
        ...
        int[] slots = {10, 11, 13, 15, 16};
        int i = 0;
        for (CrateDef crate : orderedCrates()) if (i < slots.length) inv.setItem(slots[i++], crateItem(crate));
        player.openInventory(inv);
    }
    ...
    private void openSettings(Player player) {
        ...
        int[] slots = {10, 11, 13, 15, 16};
        int i = 0;
        for (CrateDef crate : orderedCrates()) if (i < slots.length) inv.setItem(slots[i++], itemWithPdc(crate.block, "&e" + crate.name, List.of("&7Klik untuk detail setting.", "&7Effect: &f" + crate.effect, "&7Hologram: &f" + crate.hologram), crateKey, crate.id));
        player.openInventory(inv);
    }
    ...
    private void openEffect(Player player, String crateId) {
        Inventory inv = gui(new Holder("effect", crateId, null, 1), 27, "&8Effect: " + crates.get(crateId).name);
        fill(inv);
        Map<Integer, String> options = Map.of(9, "fire_spiral", 10, "cherry_spiral", 11, "end_rod_spiral", 13, "oak_leaf_spiral", 15, "smoke_spiral", 16, "bubble_spiral");
    ...
    private void openRewardRarity(Player player, String crateId) {
        Inventory inv = gui(new Holder("rarity", crateId, null, 1), 27, "&8Rewards: " + crates.get(crateId).name);
        fill(inv);
        int[] slots = {10, 11, 13, 15, 16};
        Material[] mats = {Material.WHITE_STAINED_GLASS_PANE, Material.LIME_STAINED_GLASS_PANE, Material.BLUE_STAINED_GLASS_PANE, Material.PURPLE_STAINED_GLASS_PANE, Material.ORANGE_STAINED_GLASS_PANE};
        for (int i = 0; i < RARITIES.size(); i++) inv.setItem(slots[i], itemWithPdc(mats[i], "&e" + RARITIES.get(i), List.of("&7Klik untuk edit item reward rarity ini."), keyKey, RARITIES.get(i)));
        inv.setItem(22, item(Material.ARROW, "&eBack"));
        player.openInventory(inv);
    }

    private void openRewardEditor(Player player, String crateId, String rarity) {
        Inventory inv = gui(new Holder("rewardedit", crateId, rarity, 1), 54, "&8Edit " + crateId + " " + rarity);
        List<?> items = rewardsGuiYml.getList("rewards." + crateId + "." + rarity + ".items", List.of());
        int slot = 0;
        for (Object object : items) if (object instanceof ItemStack stack && slot < inv.getSize()) inv.setItem(slot++, stack.clone());
        player.openInventory(inv);
    }
    ...
    public void onClick(InventoryClickEvent event) {
        try {
            ...
            switch (holder.type) {
                ...
                case "keys" -> { String key = pdc(clicked, keyKey); if (key != null) giveOrDrop(player, createKeyItem(keys.get(key), event.isRightClick() ? 64 : event.isShiftClick() ? 10 : 1)); }
    ...
    private void detailClick(Player player, String crateId, int slot) {
        if (slot == 10) {
            ...
            msg(player, "&aHologram crate diubah.");
    ...
    private void saveRewardEditor(Inventory inv, String crateId, String rarity) {
        ...
        for (ItemStack item : inv.getContents()) if (item != null && item.getType() != Material.AIR) items.add(item.clone());
        rewardsGuiYml.set("rewards." + crateId + "." + rarity + ".items", items);
    ...
    private void buyKey(Player player, String keyId, int amount) {
        ...
        int max = getConfig().getInt("settings.max-shop-amount", 64);
        amount = Math.max(1, Math.min(max, amount));
        ItemStack item = createKeyItem(key, amount);
        if (!canFit(player.getInventory(), item)) {
            msg(player, "&cInventory penuh. Kosongkan slot dulu.");
            return;
        }
        if (economy == null) {
            msg(player, msg("economy-missing", "&cEconomy Vault tidak tersedia."));
            return;
        }
        double total = key.price * amount;
        if (economy.getBalance(player) < total) {
            msg(player, msg("not-enough-money", "&cUang kamu kurang. Butuh &f%price%&c.").replace("%price%", cash(total)));
            return;
        }
    ...
    private boolean startRoulette(Player player, CrateDef crate, boolean preview) {
        ...
        if (openings.containsKey(player.getUniqueId())) {
            msg(player, msg("animation-running", "&cKamu sedang membuka crate."));
            return false;
        }
        Reward finalReward = chooseReward(crate.id);
        ...
        fill(inv);
        inv.setItem(13, item(Material.LIME_STAINED_GLASS_PANE, "&a▼"));
        inv.setItem(31, item(Material.LIME_STAINED_GLASS_PANE, "&a▲"));
        player.openInventory(inv);
        ...
        int closeDelay = Math.max(1, getConfig().getInt("animation.close-delay-ticks", 30));
        ...
        BukkitTask task = new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (ticks >= next) {
                    ...
                    next = ticks + delay;
                    for (int slot = 19; slot <= 25; slot++) inv.setItem(slot, chooseReward(crate.id).icon());
                    inv.setItem(22, ticks >= duration ? finalReward.icon() : chooseReward(crate.id).icon());
    ...
    private Reward chooseReward(String crate) {
        List<Reward> list = rewards.getOrDefault(crate, List.of());
        if (list.isEmpty()) return fallbackReward();
    ...
    private Reward fallbackReward() { return new Reward(RewardType.ITEM, "IRON_INGOT x8", Material.IRON_INGOT, 8, 10, List.of(), new ItemStack(Material.IRON_INGOT, 8)); }

    private void noKey(Player player, Location loc, CrateDef crate) {
        KeyDef key = keys.get(crate.key);
        msg(player, msg("no-key", "&cKamu butuh &f%key_name% &cuntuk membuka crate ini. Beli di &f/key shop&c.").replace("%key_name%", key == null ? crate.key : plain(key.name)));
        knock(player, loc);
    }

    private void wrongKey(Player player, Location loc) {
        msg(player, msg("wrong-key", "&cKey ini tidak cocok untuk crate ini."));
        knock(player, loc);
    }
    ...
    private void spawnAllHolograms() {
        ...
        for (Map.Entry<String, CrateLoc> entry : locations.entrySet()) spawnHologram(entry.getKey(), entry.getValue());
    }
    ...
    private void spawnHologram(String key, CrateLoc loc) {
        ...
        if (world == null || crate == null || !crate.hologram) return;
    ...
    private String holoText(CrateDef crate, CrateLoc loc) {
        String keysText = "-";
        Player nearest = nearest(new Location(Bukkit.getWorld(loc.world), loc.x + .5, loc.y + .5, loc.z + .5));
        ...
        if (lines.isEmpty()) lines = List.of("&d%crate_display% Crate", "&fKeys: %keys%");
        ...
        hologramTask = new BukkitRunnable() {
            @Override public void run() {
                for (Map.Entry<String, CrateLoc> entry : locations.entrySet()) {
                    ...
                    if (entity instanceof TextDisplay display && crate != null) display.setText(color(holoText(crate, entry.getValue())));
                }
            }
    ...
    private void startEffects() {
        ...
        int interval = Math.max(2, getConfig().getInt("effects.interval-ticks", 4));
        effectTask = new BukkitRunnable() {
            double t = 0;
            @Override public void run() {
                t += getConfig().getDouble("effects.speed", 0.18D);
                for (CrateLoc loc : locations.values()) effect(loc, t);
            }
    ...
    private void effect(CrateLoc loc, double t) {
        ...
        int points = Math.max(1, getConfig().getInt("effects.points-per-tick", 3));
        double radius = getConfig().getDouble("effects.radius", 0.85D);
        double height = getConfig().getDouble("effects.height", 1.6D);
        for (int i = 0; i < points; i++) {
            double a = t + (i * 2.0D * Math.PI / points);
            double y = loc.y + .25 + ((t + i * .35) % height);
            double x = loc.x + .5 + Math.cos(a) * radius;
            double z = loc.z + .5 + Math.sin(a) * radius;
            world.spawnParticle(effectParticle(crate.effect), x, y, z, 1, .03, .03, .03, .01);
            if (crate.effect.toLowerCase(Locale.ROOT).contains("fire")) world.spawnParticle(particle("LAVA", Particle.FLAME), x, y, z, 1, .02, .02, .02, 0);
        }
    }

    private Particle effectParticle(String effect) {
        String s = effect.toLowerCase(Locale.ROOT);
        if (s.contains("cherry")) return particle("CHERRY_LEAVES", Particle.HAPPY_VILLAGER);
        if (s.contains("end")) return particle("END_ROD", Particle.FLAME);
        if (s.contains("oak") || s.contains("leaf")) return Particle.HAPPY_VILLAGER;
        if (s.contains("smoke")) return particle("CAMPFIRE_COSY_SMOKE", particle("SMOKE", Particle.CLOUD));
        if (s.contains("bubble")) return particle("BUBBLE_POP", particle("BUBBLE", particle("SPLASH", Particle.HAPPY_VILLAGER)));
    ...
    private void sendDebug(CommandSender sender) {
        ...
        msg(sender, "&7hologram provider: &fTEXT_DISPLAY");
    ...
    private void createDump(CommandSender sender) {
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter(dump, false))) {
            ...
            out.println("latestLog=" + new File(logsDir, "latest.log").getPath());
    ...
    private List<CrateDef> orderedCrates() { return new ArrayList<>(crates.values()); }
    ...
    private void fill(Inventory inv) { ItemStack filler = item(mat(getConfig().getString("shop.filler"), Material.BLACK_STAINED_GLASS_PANE), " "); for (int i = 0; i < inv.getSize(); i++) inv.setItem(i, filler); }
    ...
    private boolean canFit(PlayerInventory inv, ItemStack item) { int remaining = item.getAmount(); for (ItemStack current : inv.getStorageContents()) { if (current == null || current.getType() == Material.AIR) remaining -= item.getMaxStackSize(); else if (current.isSimilar(item)) remaining -= Math.max(0, current.getMaxStackSize() - current.getAmount()); if (remaining <= 0) return true; } return false; }
    ...
    private Material mat(String name, Material fallback) { Material material = name == null ? null : Material.matchMaterial(name.toUpperCase(Locale.ROOT)); return material == null ? fallback : material; }
